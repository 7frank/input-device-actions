...
__export(core_exports, {
  Hotkeys: () => Hotkeys,
  addComboForAction: () => addComboForAction,
  getActionByName: () => getActionByName,
  getRegistered: () => getRegistered,
  isBound: () => isBound,
  isBoundTo: () => isBoundTo,
  rebind: () => rebind,
  resetActionCombosToDefault: () => resetActionCombosToDefault
});
module.exports = __toCommonJS(core_exports);

// src/core/listeners.js
var import_mutation_summary = __toESM(require("mutation-summary"));
function onElementChange(node = window.document, elem, handler) {
  var observer = new import_mutation_summary.default({
    callback: handler,
    // required
    rootNode: node,
...
function getMousetrapInstance(options) {
  var instance = new import_mousetrap.default(options.target);
  return {
    _instance: instance,
...
var _events = new import_tiny_emitter.default();
function Hotkeys(el = window) {
  ...
  return {
    on: function(action, handler, extra = null) {
      ...
      var options = (0, import_lodash.assignIn)({}, defaults);
...
Hotkeys.register = function(action, combo, options) {
  ...
  options = (0, import_lodash.assignIn)(defaults, options);
  options.action = action;
  options.combo = convertComboParams(combo);
  options.defaults = _.values((0, import_lodash.assignIn)({}, options.combo));
...
function convertComboParams(comboParams) {
  ...
  function strToObj(comboParam) {
    return (0, import_lodash.assignIn)(def(), { combo: comboParam });
  }
  ...
  comboParams = _.map(comboParams, (comboParam) => {
    ...
    else return (0, import_lodash.assignIn)(def(), comboParam);
  });
  return comboParams;
}
...
function bindSingleCombo(opt, comboParam) {
  ...
  function createCustomActionEvent(e, isFirstHandler) {
    ...
    (0, import_lodash.forEach)(el, function(val) {
...
function cloneObject(options) {
  return (0, import_lodash.assignIn)({}, options);
}
