  ...
  if (options.target == window) {
    ...
    if (!HIDocumentInstance)
      window.HI = //expose global ... as long as feedback plugin relies on it
        HIDocumentInstance =
        instance =
          new HumanInput(window, settings);
    else instance = HIDocumentInstance;
  } else instance = new HumanInput(options.target, settings);
  ...
  return {
    ...
    bind(comboParam, handlerWrapper) {
      ...
      if (!hasSecondHandler(options)) {
        ...
        console.warn(
          "the human input wrapper does not handle 'undo'-events like key/mouse- duwn/up "
        );
      }
    },
    unbind(prevCombo) {
